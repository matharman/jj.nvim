...
local function info(msg, ...)
	vim.notify(_fmt(msg, ...), vim.log.levels.INFO)
end
...
function M.open()
	local diff = require("jj.core.diff")
	local cmd = "jj diff --quiet --no-pager --color=never --git --from @-"
	local out, ok = runner.execute_command(cmd, "Failed to get changeset diff:", nil, false)
	if not ok or not out then
		return nil
	end

	local hunks = diff.parse(vim.split(out, "\n", { plain = true }))
	-- info(vim.inspect(hunks))
